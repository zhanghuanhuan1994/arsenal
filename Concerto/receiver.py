...
class Receiver(object):
    ...
    def __init__(self, port=0, buffer_size=10):
        ...
        sys.stderr.write('[Receiver] IL Listening on port %s\n' %
        ...
        if Receiver.is_multi:
            self.log_path = os.path.join(Receiver.dir, 'log', Receiver.subdir, 'IL_recv_multi.log')
            self.f = open(self.log_path, mode='w')
        else:
            self.log_path = os.path.join(Receiver.dir, 'log', Receiver.subdir, 'IL_recv_single.log')
            self.f = open(self.log_path, mode='w')
        ...
        while True:
            try:
                ...
                if data == str.encode("Hello from sender") and self.peer_addr is None:
                    self.peer_addr = addr
                    self.sock.sendto(str.encode("Hello from receiver"), self.peer_addr)
                    sys.stderr.write("[Receiver] Handshake success! IL Sender's address is %s:%s\n" % addr)
                    sys.stderr.flush()
                    return
            except IOError:
                pass
    ...
    def get_throughput(self):
        while True:
            throughput_info = "IL: " + str(self.one_second_data * 8 / 1000000.0) + " mbps\n"
            self.one_second_data = 0
...
def main(subdir=None, is_multi=False):
    ...
    try:
        ...
        print("il_receiver exception")
        print(e.args)
    finally:
        print("[receiver] IL finally")
