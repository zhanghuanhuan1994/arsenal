...
class Receiver(object):
    ...
    def __init__(self, port=0, buffer_size=10):
        ...
        sys.stderr.write('[Receiver] RL Listening on port %s\n' %
        ...
        if Receiver.is_multi:
            self.log_path = os.path.join('log', Receiver.subdir, 'RL_recv_multi.log')
            self.f = open(self.log_path, mode='w')
        else:
            self.log_path = os.path.join('log', Receiver.subdir, 'RL_recv_single.log')
            self.f = open(self.log_path, mode='w')
    ...
    def receive(self, packet):
        ...
        info = "seq:" + str(packet.seq) + " frame_id:" + str(packet.frame_id) + " send_ms:" + str(
            int(1000 * packet.send_time_ms)) + " frame_start:" + str(
            packet.frame_start_packet_seq) + " frame_end:" + str(
            packet.frame_end_packet_seq) + " recv_time:" + str(int(1000 * packet.arrival_time_ms)) + " delay:" + str(
            1000.0 * (packet.arrival_time_ms - packet.send_time_ms)) + " codec_bitrate:" + str(
    ...
    def handshake0(self):
        ...
        while True:
            try:
                ...
                if data == str.encode("Hello from sender") and self.peer_addr is None:
                    self.peer_addr = addr
                    self.sock.sendto(str.encode("Hello from receiver"), self.peer_addr)
                    sys.stderr.write("[Receiver] Handshake success! RL Sender's address is %s:%s\n" % addr)
                    sys.stderr.flush()
                    return
            except IOError:
                pass
    ...
    def get_throughput(self):
        while True:
            throughput_info = "RL receiver: " + str(self.one_second_data * 8 / 1000000.0) + " mbps\n"
            self.one_second_data = 0
...
def main(subdir=None, is_multi=False):
    ...
    try:
        ...
        print("[receiver] RL Exception")
        print(e.args)
    finally:
        print("[receiver] RL Finally")
