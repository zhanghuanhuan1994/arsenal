...
class Receiver(object):
    ...
    def __init__(self):
        ...
        self.sock.bind(('0.0.0.0', 10100))
        # self.sock.bind(('127.0.0.1', 10100))
        self.sock.listen(3)

        sys.stderr.write("[Receiver] bbr is listening on port 10100\n")
        sys.stderr.flush()
        ...
        if Receiver.is_multi:
            self.log_path = os.path.join(Receiver.dir, 'log', Receiver.subdir, 'BBR_recv_multi.log')
            self.f = open(self.log_path, mode='w')
        else:
            self.log_path = os.path.join(Receiver.dir, 'log', Receiver.subdir, 'BBR_recv_single.log')
            self.f = open(self.log_path, mode='w')
